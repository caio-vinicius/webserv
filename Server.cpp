...
ft::Server::Server(Config config): _config(config) {
    ...
    for (; i < 2; it++) {
        ...
        bind(it->second.sockfd, (struct sockaddr *)&server_addr[i],
    ...
    while (true) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            client_fd1 = accept(fds[0].fd, (struct sockaddr *)&server_addr[0],
                reinterpret_cast<socklen_t *>(&server_addr[0]));
            if (client_fd1 < 0) {
        ...
        if (fds[1].revents & POLLIN) {
            ...
            client_fd2 = accept(fds[1].fd, (struct sockaddr *)&server_addr[1],
                reinterpret_cast<socklen_t *>(&server_addr[1]));
            if (client_fd2 < 0) {
                std::cout << "Accept Failed" << std::endl;
            } else {
